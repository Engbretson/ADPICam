...
ADPICam::ADPICam(const char *portName, int maxBuffers, size_t maxMemory,
	...
	createParam(PICAM_ADSizeYFString, asynParamInt32,
		&PICAM_ADSizeYF);
	createParam(PICAM_InitCamString, asynParamInt32,
		&PICAM_InitCam);

	createParam(PICAM_SizeYString, asynParamInt32,
		&PICAM_Sizey);
	createParam(PICAM_ArraySizeYString, asynParamInt32,
		&PICAM_ArraySizeY);
	createParam(PICAM_ArraySizeString, asynParamInt32,
		&PICAM_ArraySize);
...
asynStatus ADPICam::readEnum(asynUser *pasynUser, char *strings[], int values[],
	...
	asynPrint(pasynUserSelf, ASYN_TRACE_ERROR, "*******%s:%s: entry %s\n",
		driverName, functionName, whoami);
	...
	*nIn = 0;
	if (function == PICAM_AvailableCameras) {
		...
		asynPrint(pasynUserSelf, ASYN_TRACE_ERROR, "Getting Unavailable IDs\n");
...
asynStatus ADPICam::writeFloat64(asynUser *pasynUser, epicsFloat64 value) {
		...
		if (isRelevant && currentCameraHandle != NULL) {
			...
			if (error != PicamError_None){
				...
				asynPrint(pasynUserSelf, ASYN_TRACE_ERROR,
					...
					"driver param %d (%s), picam param:%d: %s\n",
					driverName,
					functionName,
					function, whoami,
					picamParameter,
	...
	if (status) {
		asynPrint(pasynUser, ASYN_TRACE_ERROR,
			"%s:%s: error, status=%d function=%d (%s), value=%f\n", driverName,
			functionName, status, function, whoami, value);
	}
	else {

		asynPrint(pasynUser, ASYN_TRACEIO_DRIVER,
			"%s:%s: function=%d (%s), value=%f\n", driverName, functionName,
...
asynStatus ADPICam::writeInt32(asynUser *pasynUser, epicsInt32 value) {
	...
	PicamError error = PicamError_None;
	const char* errorString;
	int sizeX, sizeY = 0, binX, binY, minX, minY;
	int function = pasynUser->reason;
	PicamParameter picamParameter;
	int adStatus;
	int acquiring;
	...
	// No idea of where and why 0 comes from, beamline said said force it to positive as a working solution

	if ((function == PICAM_TriggerDetermination) && (value == 0)) {
		value = 1;
	}

	/*
	// if not in kenetics mode, thsi setting has no meaning
	if ((KineticsMode == 1) && ((function == ADTriggerMode) && (value == 3)))
	return asynSuccess; // I do not want to flag an error, just ignore the command

	// Changing SizeY directly in Kinetics mode is invalid, should be kinetics Window Height
	if ((KineticsMode != 1) && (function == ADSizeY))
	return asynSuccess; // I do not want to flag an error, just ignore the command

	// Changing BinY directly in Kinetics mode is invalid, and actually should be 1
	if ((KineticsMode != 1) && (function == ADBinY))
	return asynSuccess; // I do not want to flag an error, just ignore the command

	// Changing ROIYInput directly in Kinetics mode also has no meaning, and actually should be 0
	if ((KineticsMode != 1) && (function == ADMinY))
	return asynSuccess; // I do not want to flag an error, just ignore the command
	*/
	...
	// PIXIS Kinetics Mode Corrections and Suresh requested visual tweeks

	//	if ((function == PICAM_InitCam) || (KineticsMode != 1)) {

	if (function == PICAM_InitCam) {
		//		return (piInitializeAllCameraParameters());
		piInitializeAllCameraParameters();
	}

	// If I am in any mode and the request is to return to Fullscreen mode, restore the old SizeY parameter
	//	if (((KineticsMode != 1) && (function == PICAM_ReadoutControlMode)) && (value == 1))
	if ((function == PICAM_ReadoutControlMode) && (value == 1))
	{
		int oldvalue;
		//     getIntegerParam(PICAM_ADSizeYF, &oldvalue);
		// whatever full screen actualy is		
		WriteInteger(PICAM_ReadoutControlMode, PicamParameter_ReadoutControlMode, 1, 1);

		getIntegerParam(ADMaxSizeY, &oldvalue);
		setIntegerParam(ADSizeY, oldvalue);
		printf("SIZEY=%d", oldvalue);
		callParamCallbacks();
	...
	// If I am going into shift per triggermode from any mode, Must already be in kinetcis mode and Time stamps must be disabled first

	//		if ((KineticsMode != 1) && ((function == PICAM_ReadoutControlMode) && (value == 3)))
	if ((function == PICAM_ReadoutControlMode) && (value == 3))
	{
		WriteInteger(PICAM_ReadoutControlMode, PicamParameter_ReadoutControlMode, value, value);

		setIntegerParam(PICAM_TimeStamps, 0);
		callParamCallbacks();
	}

	// If I am changing to kinetics, PROC the kinetics window height button andthe AdSizeY Param

	if ((function == PICAM_ReadoutControlMode) && (value != 1)) {
		int kineticsY;
		WriteInteger(PICAM_ReadoutControlMode, PicamParameter_ReadoutControlMode, value, value);
		// Kinetics window height
		int sKineticsWindowHeight, hKineticsWindowHeight;
		ReadInteger(PICAM_KineticsWindowHeight, PicamParameter_KineticsWindowHeight, sKineticsWindowHeight, hKineticsWindowHeight);
		WriteInteger(PICAM_KineticsWindowHeight, PicamParameter_KineticsWindowHeight, sKineticsWindowHeight, sKineticsWindowHeight);

		//		getIntegerParam(PICAM_KineticsWindowHeight, &kineticsY);
		//			setIntegerParam(PICAM_KineticsWindowHeight, kineticsY);
		//			setIntegerParam(ADSizeY, kineticsY);
		redoROI = true;
		callParamCallbacks();
	}

	// When enabling modes, make sure that dependant options are also PROC-ed

	if (function == PICAM_TrackFrames) {
		int frameBitDepth;
		//this part doesnt work
		getIntegerParam(PICAM_FrameTrackingBitDepth, &frameBitDepth);
		//		printf("frameBitDepth %d\n", frameBitDepth);
...
err=piWriteInt32RangeType(pasynUser, value, function, picamParameter);
callParamCallbacks();
//		printf("\n In ROI Correction %d\n",err);
redoROI = true;
}
...
*/
	...
	if (status) {
		asynPrint(pasynUser, ASYN_TRACE_ERROR,
			"%s:%s: error, status=%d function=%d (%s), value=%d\n", driverName,
			functionName, status, function, whoami, value);
	}
	else {
...
asynStatus ADPICam::piAcquireStartOld(){
	...
	// getting crap values here, old hardware set defaults, so recompute from knowm hardware values;
	getIntegerParam(ADMaxSizeY, &sizeYMax);
	getIntegerParam(PICAM_FramesPerReadout, &framesPerReadout);
	numY = (sizeYMax / framesPerReadout);

	imageDims[0] = numX;
	// Multiply by framesPerReadout in for Kinetics mode used.
	imageDims[1] = numY * framesPerReadout;

	printf("\n %d %d %d %d \n", imageDims[0], numY, framesPerReadout, imageDims[1]);
...
asynStatus ADPICam::piGenerateListValuesFromCollection(
	...
	const char *functionName = "piGenerateListValuesFromCollection";
	...
	getParamName(piLookupDriverParameter(picamParam), (const char **)&whoami);
	...
	asynPrint(pasynUserSelf, ASYN_TRACE_FLOW,
		...
		functionName,
		whoami,
		piLookupDriverParameter(picamParam));
		...
		if (paramExists){
			...
			if (isRelevant &&
				...
				if (constraints->values_count == 0){
					//					printf("\n doing value Not Available stuff\n");
...
asynStatus ADPICam::piHandleParameterIntegerValueChanged(PicamHandle camera,
	...
	asynPrint(pasynUserSelf, ASYN_TRACE_FLOW,
		"%s:%s Enter %d\n",
		driverName,
		functionName, driverParameter);

	// attempt at writing a readonly parameter
	if (driverParameter == PICAM_Orientation)
		return asynError;

	// attempt at writing a readonly parameter
	if (driverParameter == PICAM_ReadoutOrientation)
		return asynError;
...
asynStatus ADPICam::piTweakROI(){
	...
	getIntegerParam(PICAM_ReadoutControlMode, &KineticsMode);
	Picam_GetParameterRoisValue(currentCameraHandle, PicamParameter_Rois, &rois);
	Picam_GetParameterRoisConstraint(currentCameraHandle, PicamParameter_Rois, PicamConstraintCategory_Capable, &roisConstraints);

	int kineticsY;
	getIntegerParam(PICAM_KineticsWindowHeight, &kineticsY);

	int binX, binY, minX, minY, sizeX, sizeY, sizeyF;
	int status = asynSuccess;
	...
	//	status |= getIntegerParam(PICAM_ADSizeYF, &sizeyF);
	getIntegerParam(ADMaxSizeY, &sizeyF);
	printf("\n%d\n", sizeyF);

	rois->roi_array->x_binning = binX;
	rois->roi_array->x = minX;
	rois->roi_array->width = sizeX;
	rois->roi_array->y_binning = binY;
	rois->roi_array->y = minY;
	rois->roi_array->height = sizeyF;

	if (KineticsMode != 1)
		rois->roi_array->height = kineticsY;
	//		rois->roi_array->height = sizeyF;
	...
	asynPrint(pasynUserSelf, ASYN_TRACE_ERROR,
		...
		"----- sizeX = %d\n----- sizeY = %d\n"
		"----- binX = %d\n----- binY = %d\n",
		rois->roi_array[0].x, rois->roi_array[0].y,
		rois->roi_array[0].width, rois->roi_array[0].height,
		rois->roi_array[0].x_binning, rois->roi_array[0].y_binning);


	err = Picam_SetParameterRoisValue(currentDeviceHandle, PicamParameter_Rois, rois);
	if (err != PicamError_None)	printf("\nERROR Setting ROI's\n"); else printf("\nROI's Set \n");

	// visual corrections get this right

	if (KineticsMode == 1) {
		// Always display the fullscreen value of SizeY 
		setIntegerParam(PICAM_Sizey, sizeY);

		// as far as I can tell ArraySizeY is also always identical to 
		setIntegerParam(PICAM_ArraySizeY, sizeY);

		// This math also uses full screen numbers

		setIntegerParam(PICAM_ArraySize, sizeX*sizeY * 2);

	}
	else {
		setIntegerParam(PICAM_Sizey, sizeyF);
		setIntegerParam(PICAM_ArraySizeY, sizeyF);
		setIntegerParam(PICAM_ArraySize, sizeX*sizeyF * 2);
...
void ADPICam::ReadInteger(int ADMode, PicamParameter PicamMode, int& ADValue, int& PiValue) {
	PicamError err;
	asynStatus err1;
	pibln paramExists;
	pibln paramRelevant;
	PicamValueType valueType;
	char *whoami1, *whoami2;


	getParamName(ADMode, (const char **)&whoami1);
	getParamName(piLookupDriverParameter(PicamMode), (const char **)&whoami2);

	Picam_DoesParameterExist(currentCameraHandle, PicamMode, &paramExists);
	if (paramExists) Picam_IsParameterRelevant(currentCameraHandle, PicamMode, &paramRelevant);
	if (paramRelevant) {
		Picam_GetParameterValueType(currentCameraHandle, PicamMode, &valueType);
//		printf("\n Value Type %d %d %d\n", paramExists, paramRelevant, valueType);
	}

	err1 = getIntegerParam(ADMode, &ADValue);
	if (err1 != PicamError_None) printf("ERROR getting Software %s %d (err=%d)\n", whoami1, ADValue, err1); else printf("Software %s Got %d\n", whoami1, ADValue);

	if ((paramExists) && (paramRelevant)) {
		err = Picam_GetParameterIntegerValue(currentCameraHandle, PicamMode, &PiValue);
		if (err != PicamError_None)	printf("ERROR getting Hardware %s %d (err=%d)\n", whoami2, PiValue, err); else printf("Hardware %s Got %d\n", whoami2, PiValue);
	}
	else
		printf("Hardware Parameter Does not exist or is not relevent %s\n", whoami2);

}
void ADPICam::ReadFloat(int ADMode, PicamParameter PicamMode, double& ADValue, double& PiValue) {
	PicamError err;
	asynStatus err1;
	pibln paramExists;
	pibln paramRelevant;
	...
	err1 = getDoubleParam(ADMode, &ADValue);
	if (err1 != PicamError_None) printf("ERROR getting Software %s %f\n", whoami1, ADValue); else printf("Software %s Got %f\n", whoami1, ADValue);

	if ((paramExists) && (paramRelevant)) {
		err = Picam_GetParameterFloatingPointValue(currentCameraHandle, PicamMode, &PiValue);
		if (err != PicamError_None)	printf("ERROR getting Hardware %s %f (err=%d)\n", whoami2, PiValue, err); else printf("Hardware %s Got %f\n", whoami2, PiValue);
	}
	else
		printf("Hardware Parameter Does not exist or is not relevent %s\n", whoami2);
...
void ADPICam::WriteInteger(int ADMode, PicamParameter PicamMode, int ADValue, int PiValue) {
	PicamError err;
	asynStatus err1;
	pibln paramExists;
	pibln paramRelevant;
...
//		printf("\n Value Type %d %d %d\n", paramExists, paramRelevant, valueType);
	...
	if ((paramExists) && (paramRelevant)) {
		err = Picam_SetParameterIntegerValue(currentCameraHandle, PicamMode, PiValue);
		if (err != PicamError_None)	printf("ERROR setting Hardware %s %d\n", whoami2, PiValue); else printf("Hardware %s Set %d\n", whoami2, PiValue);
	}
	else
		printf("Hardware Parameter Does not exist or is not relevent %s\n", whoami2);

	err1 = setIntegerParam(ADMode, ADValue);
	if (err1 != PicamError_None)	printf("ERROR setting Software %s %d\n\n", whoami1, ADValue); else printf("Software %s Set %d\n\n", whoami1, ADValue);

	callParamCallbacks();

}

void ADPICam::WriteFloat(int ADMode, PicamParameter PicamMode, double ADValue, double PiValue) {
	PicamError err;
	asynStatus err1;
	pibln paramExists;
	pibln paramRelevant;
	...
	if ((paramExists) && (paramRelevant)) {
		err = Picam_SetParameterFloatingPointValue(currentCameraHandle, PicamMode, PiValue);
		if (err != PicamError_None)	printf("ERROR setting Hardware %s %f (err=%d)\n", whoami2, PiValue, err); else printf("Hardware %s Set %f\n", whoami2, PiValue);
	}
	else
		printf("Hardware Parameter Does not exist or is not relevent %s\n", whoami2);

	err1 = setDoubleParam(ADMode, ADValue);
	if (err1 != PicamError_None)	printf("ERROR setting Software %s %f\n\n", whoami1, ADValue); else printf("Software %s Set %f\n\n", whoami1, ADValue);

	callParamCallbacks();

}
void ADPICam::ReadIntFloat(int ADMode, PicamParameter PicamMode, int& ADValue, double& PiValue, double& ADValueDouble) {
	PicamError err;
	asynStatus err1;
	pibln paramExists;
	pibln paramRelevant;
...
//		printf("\n Value Type %d %d %d\n", paramExists, paramRelevant, valueType);
	...
	Picam_GetParameterCollectionConstraint(currentCameraHandle, PicamMode, PicamConstraintCategory_Capable, &constraint);
	ADValueDouble = constraint->values_array[ADValue];
	Picam_DestroyCollectionConstraints(constraint);

	if (err1 != PicamError_None) printf("Float ERROR getting Software %s %d (%f)\n", whoami1, ADValue, ADValueDouble); else printf("Float Software %s Got %d (%f)\n", whoami1, ADValue, ADValueDouble);


	//	err = Picam_GetParameterIntegerValue(currentCameraHandle, PicamMode, &PiValue);
	//	if (err != PicamError_None)	printf("ERROR getting Hardware %s %d\n", whoami2, PiValue); else printf("Hardware %s Got %d\n", whoami2, PiValue);

	if ((paramExists) && (paramRelevant)) {
		err = Picam_GetParameterFloatingPointValue(currentCameraHandle, PicamMode, &PiValue);
		if (err != PicamError_None)	printf("Float ERROR getting Hardware %s %f (err=%d)\n", whoami2, PiValue, err); else printf("Float Hardware %s Got %f\n", whoami2, PiValue);
	}
	else
		printf("Hardware Parameter Does not exist or is not relevent %s\n", whoami2);

}
void ADPICam::ReadIntInt(int ADMode, PicamParameter PicamMode, int& ADValue, int& PiValue, int& ADValueInt) {
	PicamError err;
	asynStatus err1;
	pibln paramExists;
	pibln paramRelevant;
...
//		printf("\n Value Type %d %d %d\n", paramExists, paramRelevant, valueType);
	...
	Picam_GetParameterCollectionConstraint(currentCameraHandle, PicamMode, PicamConstraintCategory_Capable, &constraint);
	if (constraint)
		ADValueInt = constraint->values_array[ADValue];
	else {
		ADValueInt = 0;
		printf("constraint Parameter Does not exist or is not relevent %s\n", whoami2);
	}
	Picam_DestroyCollectionConstraints(constraint);

	//	printf("\n%d %d\n", ADValue, ADValueInt);

	if (err1 != PicamError_None) printf("Int ERROR getting Software %s %d (%d)\n", whoami1, ADValue, ADValueInt); else printf("Int Software %s Got %d (%d)\n", whoami1, ADValue, ADValueInt);
	...
	if ((paramExists) && (paramRelevant)) {
		err = Picam_GetParameterIntegerValue(currentCameraHandle, PicamMode, &PiValue);
		if (err != PicamError_None)	printf("Int ERROR getting Hardware %s %d (err=%d)\n", whoami2, PiValue, err); else printf("Int Hardware %s Got %d\n", whoami2, PiValue);
	}
	else
		printf("Hardware Parameter Does not exist or is not relevent %s\n", whoami2);
...
void ADPICam::WriteIntFloat(int ADMode, PicamParameter PicamMode, int ADValue, double PiValue) {
	PicamError err;
	asynStatus err1;
	pibln paramExists;
	pibln paramRelevant;
	...
	if ((paramExists) && (paramRelevant)) {
		err = Picam_SetParameterFloatingPointValue(currentCameraHandle, PicamMode, PiValue);
		if (err != PicamError_None)	printf("Float ERROR setting Hardware %s %f (err=%d)\n", whoami2, PiValue, err); else printf("Float Hardware %s Set %f\n", whoami2, PiValue);
	}
	else
		printf("\nHardware Parameter Does not exist or is not relevent %s\n", whoami2);

	err1 = setIntegerParam(ADMode, ADValue);
	if (err1 != PicamError_None)	printf("Float ERROR setting Software %s %d\n\n", whoami1, ADValue); else printf("Float Software %s Set %d\n\n", whoami1, ADValue);

	callParamCallbacks();

}

// Most of the time, get both the MEDM and the hardware values, but set them to be MEDM value
// When certain paramters in MEDM are set to N/A, may opt to use last know good hardware value

asynStatus ADPICam::piInitializeAllCameraParameters(){

	printf("\nIn the Initilize Camera Params routine \n\n");

	// Readout Control Mode
	int sKineticsMode, hKineticsMode, stemp;
	ReadInteger(PICAM_ReadoutControlMode, PicamParameter_ReadoutControlMode, sKineticsMode, hKineticsMode);
	WriteInteger(PICAM_ReadoutControlMode, PicamParameter_ReadoutControlMode, sKineticsMode, sKineticsMode);
	...
	//Frames Per Readout (Read Only)
	int sFramesPerReadout, hFramesPerReadout;
	ReadInteger(PICAM_FramesPerReadout, PicamParameter_FramesPerReadout, sFramesPerReadout, hFramesPerReadout);
	//	WriteInteger(PICAM_FramesPerReadout, PicamParameter_FramesPerReadout, sFramesPerReadout, sFramesPerReadout);

	// TriggerMode
	int sTriggerMode, hTriggerMode;
	ReadInteger(ADTriggerMode, PicamParameter_TriggerResponse, sTriggerMode, hTriggerMode);

	if (sKineticsMode == 1)
		if (sTriggerMode > 2) {
			printf("\nTrigger Mode Corrected from %d to 2\n", sTriggerMode);
			sTriggerMode = 2;
		}

	WriteInteger(ADTriggerMode, PicamParameter_TriggerResponse, sTriggerMode, sTriggerMode);

	//Trigger Related - Note: TriggerMode and TriggerResponce are the same thing

	int sShutterTimingMode, hShutterTimingMode;

	ReadInteger(PICAM_ShutterTimingMode, PicamParameter_ShutterTimingMode, sShutterTimingMode, sShutterTimingMode);

	if (sTriggerMode == 3) {
		sShutterTimingMode = 3;
		printf("\nTrigger Mode -> Trigger Responce Corrected to Always Open\n");
	}

	WriteInteger(PICAM_ShutterTimingMode, PicamParameter_ShutterTimingMode, sShutterTimingMode, sShutterTimingMode);

	// Time Stamps

	int sTimeStamps, hTimeStamps;
	ReadInteger(PICAM_TimeStamps, PicamParameter_TimeStamps, sTimeStamps, hTimeStamps);

	if (sTriggerMode == 3) {
		sTimeStamps = 0;
		printf("\nTrigger Mode -> TimeStamps Corrected to None\n");
	}
	WriteInteger(PICAM_TimeStamps, PicamParameter_TimeStamps, sTimeStamps, sTimeStamps);


	// ADC Quality

	int sADCQuality, hADCQuality;
	ReadInteger(PICAM_AdcQuality, PicamParameter_AdcQuality, sADCQuality, hADCQuality);
	WriteInteger(PICAM_AdcQuality, PicamParameter_AdcQuality, sADCQuality, sADCQuality);

	// ADC Gain
	int sADCGain, hADCGain;
	ReadInteger(PICAM_AdcAnalogGain, PicamParameter_AdcAnalogGain, sADCGain, hADCGain);
	WriteInteger(PICAM_AdcAnalogGain, PicamParameter_AdcAnalogGain, sADCGain, sADCGain);

	// ADC BitDepth
	int sADCBitDepth, hADCBitDepth;

	ReadInteger(PICAM_AdcBitDepth, PicamParameter_AdcBitDepth, sADCBitDepth, hADCBitDepth);
	WriteInteger(PICAM_AdcBitDepth, PicamParameter_AdcBitDepth, sADCBitDepth, sADCBitDepth);

	//Query ADC Speed capabilities, messier since I get an int index into a list from MEDM but I actually have to send a double value back

	int sADCSpeed, hADCSpeed;
	double sADCSpeedDouble = 0.0, hADCSpeedDouble = 0.0;

	//	ReadInteger(PICAM_AdcSpeed, PicamParameter_AdcSpeed, sADCSpeed, hADCSpeed);
	//	WriteInteger(PICAM_AdcSpeed, PicamParameter_AdcSpeed, sADCSpeed, sADCSpeed);
	ReadIntFloat(PICAM_AdcSpeed, PicamParameter_AdcSpeed, sADCSpeed, hADCSpeedDouble, sADCSpeedDouble);
	WriteIntFloat(PICAM_AdcSpeed, PicamParameter_AdcSpeed, sADCSpeed, sADCSpeedDouble);

	//acquire time

	double sacquireTime, hacquireTime;
	ReadFloat(ADAcquireTime, PicamParameter_ExposureTime, sacquireTime, hacquireTime);
	WriteFloat(ADAcquireTime, PicamParameter_ExposureTime, sacquireTime, sacquireTime * 1000.0);

	//Temp

	double sTemperature, hTemperature;

	ReadFloat(ADTemperature, PicamParameter_SensorTemperatureSetPoint, sTemperature, hTemperature);
	WriteFloat(ADTemperature, PicamParameter_SensorTemperatureSetPoint, sTemperature, sTemperature);



	/*
	//	ReadInteger(PICAM_ShutterTimingMode, PicamParameter_ShutterTimingMode, sTriggerMode, hTriggerMode);

	// If TriggerMode is No Responce (1), no restrictions
	// If TriggerMode is readout per trigger (2), PICAM shutter mode can not be Normal
	// If TriggerMode is Shift Per trigger (3), TimeStamps Must be Turned Off, ShutterControl can not be Normal, have to be in kinetics mode
	// this also has to be validated in writeint32

	if (sTriggerMode > 1){
	int sShutterTimingMode, hShutterTimingMode;
	ReadInteger(PICAM_ShutterTimingMode, PicamParameter_ShutterTimingMode, sShutterTimingMode, hShutterTimingMode);
	WriteInteger(PICAM_ShutterTimingMode, PicamParameter_ShutterTimingMode, 3, 3);
	printf("\n correcting trigger modes \n");
	}

	if (sTriggerMode > 2){
	int sTimeStamps, hTimeStamps;
	ReadInteger(PICAM_TimeStamps, PicamParameter_TimeStamps, sTimeStamps, hTimeStamps);
	WriteInteger(PICAM_TimeStamps, PicamParameter_TimeStamps, 0, 0);

	if (sKineticsMode == 1) {
	sTriggerMode = 2;
	printf("\n correcting trigger modes \n");
	WriteInteger(PICAM_ShutterTimingMode, PicamParameter_ShutterTimingMode, 2, 2);
	...
	*/
	...
	/*
	// Only valid if  triggermode is not No Responce (1), shutter control can not be Normal, but handled above

	if (sTriggerMode != 1) {
	int sTriggerDetermination, hTriggerDetermination;
	ReadInteger(PICAM_TriggerDetermination, PicamParameter_TriggerDetermination, sTriggerDetermination, hTriggerDetermination);
	WriteInteger(PICAM_TriggerDetermination, PicamParameter_TriggerDetermination, sTriggerDetermination, sTriggerDetermination);
	...
	*/
	...
	int sTrackFrames, hTrackFrames;
	ReadInteger(PICAM_TrackFrames, PicamParameter_TrackFrames, sTrackFrames, hTrackFrames);
	WriteInteger(PICAM_TrackFrames, PicamParameter_TrackFrames, sTrackFrames, sTrackFrames);
	...
	// Pre, Post shutter compensation - variable support in hardware on this camera

	int sShutterDelayResolution, hShutterDelayResolution;
	double sShutterDelayResolutionDouble = 0.0, hShutterDelayResolutionDouble = 0.0;

	ReadIntFloat(PICAM_ShutterDelayResolution, PicamParameter_ShutterDelayResolution, sShutterDelayResolution, hShutterDelayResolutionDouble, sShutterDelayResolutionDouble);
	WriteIntFloat(PICAM_ShutterDelayResolution, PicamParameter_ShutterDelayResolution, sShutterDelayResolution, sShutterDelayResolutionDouble);

	double sShutterOpenDelay, hShutterOpenDelay;
//	ReadFloat(ADShutterOpenDelay, PicamParameter_ShutterOpeningDelay, sShutterOpenDelay, hShutterOpenDelay);
//	WriteFloat(ADShutterOpenDelay, PicamParameter_ShutterOpeningDelay, sShutterOpenDelay, sShutterOpenDelay);

	ReadFloat(PICAM_ShutterOpeningDelay, PicamParameter_ShutterOpeningDelay, sShutterOpenDelay, hShutterOpenDelay);
	WriteFloat(PICAM_ShutterOpeningDelay, PicamParameter_ShutterOpeningDelay, sShutterOpenDelay, sShutterOpenDelay);

	double sShutterCloseDelay, hShutterCloseDelay;
	int sShutterCloseDelayInt;
//	ReadFloat(ADShutterCloseDelay, PicamParameter_ShutterClosingDelay, sShutterCloseDelay, hShutterCloseDelay);
//	WriteFloat(ADShutterCloseDelay, PicamParameter_ShutterClosingDelay, sShutterCloseDelay, sShutterCloseDelay);

	ReadFloat(PICAM_ShutterClosingDelay, PicamParameter_ShutterClosingDelay, sShutterCloseDelay, hShutterCloseDelay);
	sShutterCloseDelayInt = sShutterCloseDelay;
	if (sShutterCloseDelayInt % 2 == 0)
		;
	else sShutterCloseDelay += 1.0;

	WriteFloat(PICAM_ShutterClosingDelay, PicamParameter_ShutterClosingDelay, sShutterCloseDelay, sShutterCloseDelay);

	//pass 2, not all values are legal, if they are not, I want to reflect the bad value
	//	ReadFloat(ADShutterCloseDelay, PicamParameter_ShutterClosingDelay, sShutterCloseDelay, hShutterCloseDelay);
	//	WriteFloat(ADShutterCloseDelay, PicamParameter_ShutterClosingDelay, hShutterCloseDelay, hShutterCloseDelay);
	// do RBV on screen to reflect the incorrect state
	...
	int sOutputSignal, hOutputSignal;
	ReadInteger(PICAM_OutputSignal, PicamParameter_OutputSignal, sOutputSignal, hOutputSignal);
	WriteInteger(PICAM_OutputSignal, PicamParameter_OutputSignal, sOutputSignal, sOutputSignal);

	// Readout Orientation Read Only

	int sReadoutOrientation, hReadoutOrientation;
	ReadInteger(PICAM_ReadoutOrientation, PicamParameter_ReadoutOrientation, sReadoutOrientation, hReadoutOrientation);
	//	WriteInteger(PICAM_ReadoutOrientation, PicamParameter_ReadoutOrientation, sReadoutOrientation, sReadoutOrientation);
	printf("\n");

	// ReadoutTime CalculationRead Only readout time per image in milliseconds PIXIS about 0.87516 seconds in fast mode, 17.42416 secs slow mode.

	double sReadoutTimeCalculation, hReadoutTimeCalculation;
	ReadFloat(PICAM_ReadoutTimeCalc, PicamParameter_ReadoutTimeCalculation, sReadoutTimeCalculation, hReadoutTimeCalculation);
	//	WriteFloat(PICAM_ReadoutTimeCalc, PicamParameter_ReadoutTimeCalculation, sReadoutTimeCalculation, sReadoutTimeCalculation);
	printf("\n");

	// Vertical Shift rate

	int sVerticalShiftRate, hVerticalShiftRate;
	double sVerticalShiftRateDouble, hVerticalShiftRateDouble;

	ReadIntFloat(PICAM_VerticalShiftRate, PicamParameter_VerticalShiftRate, sVerticalShiftRate, hVerticalShiftRateDouble, sVerticalShiftRateDouble);
	WriteIntFloat(PICAM_VerticalShiftRate, PicamParameter_VerticalShiftRate, sVerticalShiftRate, sVerticalShiftRateDouble);

	// TimeStampBitDepth

	int sTimeStampBitDepth, hTimeStampBitDepth;
	int sTimeStampBitDepthInt;

	ReadIntInt(PICAM_TimeStampBitDepth, PicamParameter_TimeStampBitDepth, sTimeStampBitDepth, hTimeStampBitDepth, sTimeStampBitDepthInt);
	WriteInteger(PICAM_TimeStampBitDepth, PicamParameter_TimeStampBitDepth, sTimeStampBitDepth, sTimeStampBitDepthInt);


// state machine constraints/conflicts/rules
// If ReadoutMode = FullScreen, TriggerMode can not Be ShiftPerTrigger (3), set it to ReadoutPerTrigger(2)
//If triggerMode is ReadoutPerTrigger(3), TimeStamps Must be disabled



	piTweakROI();

	// allocates a global data buffer
	callParamCallbacks();
	return  asynSuccess;
}
...
int trackframes;
getIntegerParam(PICAM_TrackFrames, &trackframes);
err1 = setIntegerParam(PICAM_TrackFrames, trackframes);
if (err1 != asynSuccess)printf("ERROR setting TrackFrames %d\n", trackframes); else printf("TrackFrames Set %d\n", trackframes);

// No real reason to do this. For PIXIS, all constants that I can not actually change.

if (trackframes > 0) {

	// Need to get this actually from the camera since the AD param values display as one thing, but read back as another.

	int frameBitDepth;
	//		Picam_GetParameterIntegerValue(currentCameraHandle, PicamParameter_FrameTrackingBitDepth, &frameBitDepth);
	getIntegerParam(PICAM_FrameTrackingBitDepth, &frameBitDepth);
	err1 = setIntegerParam(PICAM_FrameTrackingBitDepth, frameBitDepth);
	if (err1 != asynSuccess)printf("ERROR setting FrameTrackingBitDepth %d\n", frameBitDepth); else printf("FrameTrackingBitDepth Set %d\n", frameBitDepth);

}


int timestamps;
getIntegerParam(PICAM_TimeStamps, &timestamps);
err1 = setIntegerParam(PICAM_TimeStamps, timestamps);
if (err1 != asynSuccess)printf("ERROR setting TimeStamps %d\n", timestamps); else printf("TimeStamps Set %d\n", timestamps);
...
if (timestamps > 0) {
	...
	int timeBitDepth;
	//		Picam_GetParameterIntegerValue(currentCameraHandle, PicamParameter_TimeStampBitDepth, &timeBitDepth);
	getIntegerParam(PicamParameter_TimeStampBitDepth, &timeBitDepth);
	err1 = setIntegerParam(PICAM_TimeStampBitDepth, timeBitDepth);
	if (err1 != asynSuccess)printf("ERROR setting TimeStampBitDepth %d\n", timeBitDepth); else printf("TimeStampBitDepth Set %d\n", timeBitDepth);

	int timeResolution;
	//		Picam_GetParameterIntegerValue(currentCameraHandle, PicamParameter_TimeStampResolution, &timeResolution);
	getIntegerParam(PICAM_TimeStampResolution, &timeResolution);
	err1 = setIntegerParam(PICAM_TimeStampResolution, timeResolution);
	if (err1 != asynSuccess)printf("ERROR setting TimeStampResolution %d\n", timeResolution); else printf("TimeStampResolution Set %d\n", timeResolution);
...
piint failCount;
err = Picam_CommitParameters(currentCameraHandle, &paramsFailed, &failCount);
if (err != PicamError_None)		printf("ERROR setting CommitParams %d\n", failCount); else printf("CommitParams Set %d\n", failCount);
