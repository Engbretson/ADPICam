...
#ifdef QWERTY
/**
* Override asynPortDriver's writeOctet method.
*/
 asynStatus ADPICam::writeOctet(asynUser *pasynUser, const char *value,
	size_t nChars, size_t *nActual)
{
	int addr = 0;
	int function = pasynUser->reason;
...
// does not predicavly work
	/*
	// If I am in Full screen mode  and change SizeY, Save copy of valid SizeY 
	if ((KineticsMode == 1) && (function == ADSizeY)) {
		int ksizeY;
		getIntegerParam(ADSizeY, &ksizeY);
		setIntegerParam(PICAM_ADSizeYF, ksizeY);
		printf("\nSaving Full screen height %d\n", ksizeY);
	}

	// If I am in Full screen mode  and going into A Kinetics mode Save the current sizey 
	if ((KineticsMode == 1) && (function == PICAM_ReadoutControlMode)) {
		int ksizeY;
		getIntegerParam(ADSizeY, &ksizeY);
		setIntegerParam(PICAM_ADSizeYF, ksizeY);
		printf("\nSaving Full screen height %d\n", ksizeY);
...
*/
	...
	// If I am in kinetics mode and the request is to return to Fullscreen mode, restore the old SizeY parameter
...
//     getIntegerParam(PICAM_ADSizeYF, &oldvalue);
		...
		callParamCallbacks();
		printf("\nRestoring  Full screen height %d\n", oldvalue);
	}

	
	// If I am going into shift per triggermode, Must already be in kinetcis mode and Time stamps must be disabled first
	...
	// If I am changing to kinetics, PROC the kinetics window height button
	...
	if ((function == PICAM_ReadoutControlMode) && (value != 1)) {
		int kineticsY;
			...
			callParamCallbacks();
			printf("\nPROC-ing Kinetics Window height %d", kineticsY);
	}
...
asynStatus ADPICam::piInitializeAllCameraParameters(){
	...
	/*
	pvcam does the following, note - writing first to the actual hardware, then to AD params
    Getting all the data from AD paramters

Writing to the AD paramters effectively also pushed down to the hardware.

	PVCamDetectorMode -> PICAM_ReadoutControlMode
	PVCamSpeedTableIndex ??? ADC speed ??? if so, do all the ADC params, just for completeness
	PVCamGainIndex
	PVCamSetTemperature, if available
	PVCamTriggerEdge, if available, EDGE_TRIG_POS or EDGE_TRIG_NEG
	// TTL Output Logic (Commented out)
	PVCamShutterOpenDelay, if available
	PVCamShuuerCloseDelay, if available
	ROI's get sizes
	Allocate data based upon ROI's
	// setup Callbacks (Commented out)
	PVCamTRiggerMode_RBV
	CallParamCallbacks
	*/
...
//Mode
	...
	err1 = getIntegerParam(PICAM_ReadoutControlMode, &KineticsMode);
	if (err1 != asynSuccess) printf("Can Not Read AD Parameter\n");

//	err = Picam_SetParameterIntegerValue(currentCameraHandle, PicamParameter_ReadoutControlMode, KineticsMode);
//	if (err != PicamError_None)	printf("ERROR setting Kinetics Mode %d\n", KineticsMode); else printf("Kinetics Mode Set %d\n", KineticsMode);
	
	err1 = setIntegerParam(PICAM_ReadoutControlMode, KineticsMode);
	if (err1 != PicamError_None)	printf("ERROR setting Kinetics Mode %d\n", KineticsMode); else printf("Kinetics Mode Set %d\n", KineticsMode);
...
//ADC
	...
	err1 = getIntegerParam(PICAM_AdcAnalogGain, &ADCGain);
	if (err1 != asynSuccess) printf("Can Not Read AD Parameter\n");

	err1 = setIntegerParam(PICAM_AdcAnalogGain, ADCGain);
	if (err1 != PicamError_None)	printf("ERROR setting ADCGain %d\n", ADCGain); else printf("ADCGain Set %d\n", ADCGain);

	int ADCBitDepth;

	err1 = getIntegerParam(PICAM_AdcBitDepth, &ADCBitDepth);
	if (err1 != asynSuccess) printf("Can Not Read AD Parameter\n");

	err1 = setIntegerParam(PICAM_AdcBitDepth, ADCBitDepth);
	if (err1 != PicamError_None)	printf("ERROR setting ADCBitDepth %d\n", ADCBitDepth); else printf("ADCBitDepth Set %d\n", ADCBitDepth);

	int ADCSpeed;

	err1 = getIntegerParam(PICAM_AdcSpeed, &ADCSpeed);
	if (err1 != asynSuccess) printf("Can Not Read AD Parameter\n");

	err1 = setIntegerParam(PICAM_AdcSpeed, ADCSpeed);
	if (err1 != PicamError_None)	printf("ERROR setting ADCSpeed %d\n", ADCSpeed); else printf("ADCSpeed Set %d\n", ADCSpeed);


	int ADCQuality;

	err1 = getIntegerParam(PICAM_AdcQuality, &ADCQuality);
	if (err1 != asynSuccess) printf("Can Not Read AD Parameter\n");

	err1 = setIntegerParam(PICAM_AdcQuality, ADCQuality);
	if (err1 != PicamError_None)	printf("ERROR setting ADCQuality %d\n", ADCQuality); else printf("ADCQuality Set %d\n", ADCQuality);

//Temp

	double Temperature;

		err1 = getDoubleParam(ADTemperature, &Temperature);
	if (err1 != asynSuccess) printf("Can Not Read AD Parameter\n");

	err1 = setDoubleParam(ADTemperature, Temperature);
	if (err1 != PicamError_None)	printf("ERROR setting Temperature %f\n", Temperature); else printf("Temperature Set %f\n", Temperature);
...
//Trigger Related - Note TriggerMode and TriggerResponce are the same thing
	...
	err1 = getIntegerParam(ADTriggerMode, &TriggerMode);
	if (err1 != asynSuccess) printf("Can Not Read AD Parameter\n");

// If TriggerMode is Shift Per trigger (3), TimeStamps Must be Turned Off, ShutterControl can not be Normal.

//PLACEHOLDER

	err1 = setIntegerParam(ADTriggerMode, TriggerMode);
	if (err1 != PicamError_None)	printf("ERROR setting TriggerMode %d\n", TriggerMode); else printf("TriggerMode Set %d\n", TriggerMode);

// Only valid if  triggermode is not No Responce (1), shutter control can not be Normal
	int TriggerDetermination;

	err1 = getIntegerParam(PICAM_TriggerDetermination, &TriggerDetermination);
	if (err1 != asynSuccess) printf("Can Not Read AD Parameter\n");

	err1 = setIntegerParam(PICAM_TriggerDetermination, TriggerDetermination);
	if (err1 != PicamError_None)	printf("ERROR setting TriggerDetermination %d\n", TriggerDetermination); else printf("TriggerDetermination Set %d\n", TriggerDetermination);


// Pre, Post shutter compensation
	double ShutterOpenDelay;

	err1 = getDoubleParam(ADShutterOpenDelay, &ShutterOpenDelay);
	if (err1 != asynSuccess) printf("Can Not Read AD Parameter\n");

	err1 = setDoubleParam(ADShutterOpenDelay, ShutterOpenDelay);
	if (err1 != PicamError_None)	printf("ERROR setting ShutterOpenDelay %f\n", ShutterOpenDelay); else printf("ShutterOpenDElay Set %f\n", ShutterOpenDelay);

	double ShutterCloseDelay;

	err1 = getDoubleParam(ADShutterCloseDelay, &ShutterCloseDelay);
	if (err1 != asynSuccess) printf("Can Not Read AD Parameter\n");

	err1 = setDoubleParam(ADShutterCloseDelay, ShutterCloseDelay);
	if (err1 != PicamError_None)	printf("ERROR setting ShutterCloseDelay %f\n", ShutterCloseDelay); else printf("ShutterCloseDElay Set %f\n", ShutterCloseDelay);

// Roi tweaks may need to be looked at from multiple locations
// Note that this also should make sure than in any kinetics mode the sizeY *is* the kineticsWindow Height
...
asynStatus ADPICam::piInitializeAllCameraParameters(){
	...
	printf("In the Initilize Camera Params routine \n");
	...
	*/
	...
	/////////
	/*
	Set the readout mode , i.e. Kinetics -ok
	set the expsoure time -ok
	set the kinetics window height -ok
	set the trigger mode/responce -ok None, readout per trigger, shift per trigger
	set the trigger determination -ok
	set the ROI
	assorted other kinetics
	Commit all the parameters -0k
	Aquire, if required
	/////////
	*/
	...
	err = Picam_SetParameterIntegerValue(currentDeviceHandle, PicamParameter_ReadoutControlMode, KineticsMode);
	if (err != PicamError_None)	printf("ERROR setting Kinetics Mode %d\n", KineticsMode); else printf("Kinetics Mode Set %d\n", KineticsMode);

	double acquireTime;
	getDoubleParam(ADAcquireTime, &acquireTime);
	err = Picam_SetParameterFloatingPointValue(currentDeviceHandle, PicamParameter_ExposureTime, acquireTime*1000.0);
	if (err != PicamError_None)	printf("ERROR setting acquire time %f\n", acquireTime); else printf("Acquire Time Set %f\n", acquireTime);

	int kineticsY;
	getIntegerParam(PICAM_KineticsWindowHeight, &kineticsY);
	err = Picam_SetParameterIntegerValue(currentDeviceHandle, PicamParameter_KineticsWindowHeight, kineticsY);
	if (err != PicamError_None)	printf("ERROR setting Kinetics Window Height %d\n", kineticsY); else printf("Kinetics Window Height Set %d\n", kineticsY);

	piint triggerResponse = 0;

	err1 = getIntegerParam(ADTriggerMode, &triggerResponse);
	printf("err1 = %d\n", err1);


	err = Picam_SetParameterIntegerValue(currentCameraHandle, PicamParameter_TriggerResponse, triggerResponse);
	if (err != PicamError_None)	printf("ERROR setting Trigger Mode/Responce %d\n", triggerResponse); else printf("Trigger Mode/Responce Set %d\n", triggerResponse);

	if (triggerResponse > 1) {

		printf("Trigger Mode/Responce Additional - Readout or Shift per trigger\n");

		piint triggerDetermination;
		getIntegerParam(PicamParameter_TriggerDetermination, &triggerDetermination);
		err = Picam_SetParameterIntegerValue(currentDeviceHandle, PicamParameter_TriggerDetermination, triggerDetermination);
		if (err != PicamError_None)	printf("ERROR setting Trigger Determination %d\n", triggerDetermination); else printf("Trigger Determination Set %d\n", triggerDetermination);

	}
	else
		printf("Trigger Mode/Responce Additional, not required  - Readout or Shift per trigger\n");
