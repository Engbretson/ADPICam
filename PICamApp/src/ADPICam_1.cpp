...
ADPICam::ADPICam(const char *portName, int maxBuffers, size_t maxMemory,
	...
	createParam(PICAM_ArraySizeKString, asynParamInt32,
		&PICAM_ArraySizeK);
	createParam(PICAM_ReadoutStrideKString, asynParamInt32,
		&PICAM_ReadoutStrideK);
...
/* // doesn't always work so still possible to have wrong mode values
if (function == PICAM_ReadoutControlMode) {
		//going to Kinetics, save sizey
		if (value == 3) {
			int ksizeY;
		...
		//returning from Kinetics, restore sizey
		if (value == 1) {
                int ksizeX, ksizeY, kbinX, kbinY, kminX, kminY;
			getIntegerParam(PICAM_ADSizeYF, &ksizeY);
			setIntegerParam(ADSizeY, ksizeY);
                ...
                piSetRois(kminX, kminY, ksizeX, ksizeY, kbinX, kbinY);
                callParamCallbacks();
				...
				*/
				{
					...
					if (err != PicamError_None)	printf("ERROR in #0\n");

					double acquireTime;
					getDoubleParam(ADAcquireTime, &acquireTime);
					err = Picam_SetParameterFloatingPointValue(currentDeviceHandle, PicamParameter_ExposureTime, acquireTime);
					if (err != PicamError_None)	printf("ERROR in #1\n");						
					

					err=Picam_SetParameterIntegerValue(currentDeviceHandle, PicamParameter_KineticsWindowHeight, kineticsY);
					if (err != PicamError_None)	printf("ERROR in #2\n");

					piint triggerResponse;
					getIntegerParam(PicamParameter_TriggerResponse, &triggerResponse);

					if (triggerResponse > 0) {

						err = Picam_SetParameterIntegerValue(currentDeviceHandle, PicamParameter_TriggerResponse, triggerResponse);
						if (err != PicamError_None)	printf("ERROR in #3");
						...
						if (err != PicamError_None)	printf("ERROR in #4\n");
...
//					rois->roi_array->height = 0;
//					rois->roi_array->width = 0;
//					rois->roi_array->x = 0;
					rois->roi_array->y = kineticsY;
//					rois->roi_array->x_binning = 0;
//					rois->roi_array->y_binning = 0;

//					err=Picam_SetParameterRoisValue(currentDeviceHandle, PicamParameter_Rois, rois);
//					if (err != PicamError_None)	printf("ERROR in #5\n");

					const PicamParameter *paramsFailed;
					piint failCount;
					err=Picam_CommitParameters(currentDeviceHandle, &paramsFailed, &failCount);
					if (err != PicamError_None)	printf("ERROR in #6\n");

					Picam_DestroyParameters(paramsFailed);

					Picam_SetParameterRoisValue(currentCameraHandle, PicamParameter_Rois, rois);
...
//mse put pack the old values attempt
//				ksizeY = 1300;
//				piSetRois(kminX, kminY, ksizeX, ksizeY, kbinX, kbinY);
//				callParamCallbacks();
            }
...
//mse tag TODO
	if (KineticsMode == PicamReadoutControlMode_Kinetics) {
		getIntegerParam(PICAM_KineticsWindowHeight, &numY);
	}
	else
	{
